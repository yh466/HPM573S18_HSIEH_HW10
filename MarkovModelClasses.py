...
class PatientCostUtilityMonitor:
    ...
    def update(self, k, current_state, next_state):
        ...
        if current_state is P.HealthStats.WELL:
            cost += 0
        elif current_state is P.HealthStats.STROKE and next_state is P.HealthStats.POST_STROKE:  # half cycle correction
            cost += 0.5*self._param.get_annual_treatment_cost()*self._param.get_delta_t()
        elif current_state is P.HealthStats.POST_STROKE and next_state is P.HealthStats.DEATH:   # half cycle correction
            cost += 0.5*self._param.get_annual_treatment_cost()*self._param.get_delta_t()
        elif current_state is P.HealthStats.POST_STROKE and next_state is P.HealthStats.STROKE:  # half cycle correction
            cost += 0.5*self._param.get_annual_treatment_cost()*self._param.get_delta_t()
        else:
